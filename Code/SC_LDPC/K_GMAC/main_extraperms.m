...
function errMat=main_extraperms(Ka,Kb,collision_flag,collision_prb)
%clear all
addpath('../PEG/')
...
if collision_flag
    rand_float=rand;
    if rand_float<collision_prb
        interl(:,Ka)=interl(:,round(Ka/2)); 
    end
end
chan='Gaussian';
...
for sigIter=1:length(sigVec)
    ...
    tic
    while num_err<10 && sim_cnt<maxSims
        c=zeros(N,Ka);
        x=c;
        for u=1:Ka
    ...
    xhat=BP_KGMAC_extraperms(y,Vcon,Ccon,eMax,maxIters,P,sig);
...
%------Book-keeping for Errors--------
    ...
    fprintf('#err users/total= %d/%d---%3.2f sec\n',num_err,Ka*sim_cnt,toc);
    errMat(sigIter,1:5)=[num_err sim_cnt EsN0_vec(sigIter) Ka Kb ];
end