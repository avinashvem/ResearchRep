...
% col_weight=dv;
% row_weight=dc; % row weight is a constant always.
...
Emax=sum( var_deg_vec);
Vcon=(Emax+1)*ones(n,max(var_deg_vec));
Ccon=(Emax+1)*ones(l,max(check_deg_vec));

e=0;
chk_deg=zeros(1,l);

for i=1:n%sum(var_deg_vec)
    Vcon(i,1:var_deg_vec(i))=e+1:e+var_deg_vec(i);
    Chk_i=find(H(:,i));
    for j =1:length(Chk_i)
       Ccon(Chk_i(j),chk_deg(Chk_i(j))+1)=e+j; 
       chk_deg(Chk_i(j))=chk_deg(Chk_i(j))+1;
    end
    e=e+var_deg_vec(i);
    assert(var_deg_vec(i)==numel(Chk_i))
end
