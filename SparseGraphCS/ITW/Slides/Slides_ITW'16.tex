...
\begin{itemize}
\item Sample complexity of $O(K)$ and measurement complexity of $O(N\log(\frac{N}{K}))$ for noiseless setting
\end{itemize}
...
\only<3>{
\begin{figure}
\scalebox{3}{\input{../../Figures/SensingGraph}}
\end{figure}
}
\end{column}
...
\begin{figure}
\scalebox{5}{\input{../../Figures/PeelingAnimation}}
\end{figure}
\end{column}
...
\end{block}
\end{frame}
%R(e^{-\frac{A_{\text{min}}^{2}}{2\sigma^2}})
