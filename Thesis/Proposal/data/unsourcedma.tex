\documentclass[final,onecolumn,12pt]{IEEEtran}
\input{preamble}
\def\Ktot{K_{\text{tot}}}
\def\Ka{K_{\text{a}}}
...
While Ordentlich and Polyanskiy have contributed an important first step in finding practical schemes for the unsourced MAC, there remains a substantial gap between the performance of their proposed scheme and the capacity limit derived in~\cite{polyanskiy17}. Indeed, they point to this gap and discuss possibilities for improving its performance. In \cite[Section~III.A]{ordentlich17}, they discuss the possibility of improving their scheme by decoding the $T$ messages using the real sum from the channel output instead of first reducing the output of the channel to modulo-$p$ operations. However, in the unsourced MAC, each user is forced to use the same codebook and they remark that ``the task of designing low complexity capacity approaching same-codebook schemes for the real binary adder seems quite challenging.'' Another important limitation that is not discussed in \cite{ordentlich17} is that their scheme does not admit iterative cancellation and, hence, successive interference cancellation is not considered.
...
\end{itemize}
While \cite{ordentlich17} also incorporates the user symmetry aspect described above, our scheme differs from theirs in the other features highlighted above.
...
\begin{figure}[h]
  \centering
  \resizebox{0.9\textwidth}{!}{\input{../Figures/system_diagram}}
...
\begin{figure}[h!]
  \centering
  \resizebox{0.55\textwidth}{!}{\input{../Figures/UnsMAC_encoder.tex}}
...
\begin{figure}[h!]
  \centering
  \resizebox{0.5\textwidth}{!}{\input{../Figures/decodergraph_permutation.tex}}
...
\begin{figure}[h!]
  \centering
  \resizebox{0.75\textwidth}{!}{\input{../Figures/BP_graphs.tex}}
...
%\sum_{j=1}^{T}\vec{a}_{i_j}&\in \Lambda ~~ i_j\in[1:M_\mathrm{p}]
%\end{align*}
%where $\Lambda =\{\mathbf{G}\mathbf{u},\mb{u}\in \mbb{Z}^{m}\}, \mb{G}$ is the generator matrix of the binary code $\mc{C}_{\text{lin}}$. 
...
\begin{figure}[h!]
  \centering
  \resizebox{0.6\textwidth}{!}{\input{../Figures/Tpeeling_DEvBP.tex}}
...
\begin{figure}[h]
\centering
 \resizebox{0.6\textwidth}{!}{\input{../Figures/sim_temp.tex}}
...
\begin{figure}[h]
\centering
 \resizebox{0.6\textwidth}{!}{\input{../Figures/sim_results_30000_0.04_0.01.tex}}
...
%\begin{figure}[h]
%  \centering
%  \resizebox{0.65\textwidth}{!}{\input{../Figures/LDPC_two_users.tex}}
