...
\node[block] (r11) at (0,0){\fsize \textbf{Encoder} $\tilde{\mc{C}}$};
\node[block] (r21) at (\xr,0) {\fsize \begin{tabular}{c}
Repeat \\
$L_{w_1}$ times
\end{tabular}};

\node[block] (r12) at (0,-\ya){\fsize \textbf{Encoder} $\tilde{\mc{C}}$};
\node[block] (r22) at (\xr,-\ya) {\fsize \begin{tabular}{c}
Repeat \\
$L_{w_i}$ times
\end{tabular}};

\node[block] (r13) at (0,-3*\ya){\fsize \textbf{Encoder} $\tilde{\mc{C}}$};
...
\draw[->, thick, line width=2] (r11.east)--node[midway, above]{\fsize $\vec{\tilde{x}}_{w_1}$}(r21.west);

\draw[<-, thick, line width=2] (r12.west)--node[midway, above]{\fsize $w_i$}+(-\xoffs,0);
\draw[->, thick, line width=2] (r12.east)--node[midway, above]{\fsize $\vec{\tilde{x}}_{w_i}$}(r22.west);

\draw[<-, thick, line width=2] (r13.west)--node[midway, above]{\fsize $w_{K_a}$}+(-\xoffs,0);
\draw[->, thick, line width=2] (r13.east)--node[midway, above]{\fsize $\vec{\tilde{x}}_{w_{K_a}}$}(r23.west);
