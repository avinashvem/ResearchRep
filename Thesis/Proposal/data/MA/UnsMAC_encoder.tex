...
\node[rectangle, draw, minimum width=2.7in,thick] (codeword) at (3+0.5\ext,1+2*\ext) {\fsizes $\pi_{\tau_{\wpdash}}(\vec{c}_{\wc})$};
...
\draw[->](CWperm.north)-- (CWperm.north |- codeword.south) node[midway, right]{\fsizes $\pi_{\tau_{\wpdash}}(\cv_{\wc})$};
...
\node [above] at (codeword.north) {$\cv_{w}$};

\draw [->] (msg.north) -- (msg.north |- CWperm.west)node[midway,left]{\fsizes $\wpdash$} -- (CWperm.west) node[midway,above] {\fsizes $\tau_{\wpdash}=f(\wpdash)$};
\end{tikzpicture} 