...
% Middle bits= Actual GMAC Code bits
    ...
    \draw[\linew] (v1\i.south) -- +(270:\exta);      \draw[\linew] (v2\i.south) -- +(270:\exta);    
    \draw[\linew] (v3\i.north) -- +(90:\exta);        \draw[\linew] (v4\i.north) -- +(90:\exta);
...
% Bottom checks to top
  ...
  \node[bigPerm] (perm1_node) at (18,0.5*\vny+0.5*\cny) {\normalsize $\pi_{\text{LDPC}}$};    
  \node[bigPerm] (perm2_node) at (18,0.5*\vnyb+0.5*\ccny) {\normalsize $\pi_{\text{LDPC}}$};    
  
  \node[bigPerm] (perm3_node) at (18,0.7*\vny+0.3*\vnya) {\Large $\pi_{f(w_2^{\mathrm{p}})}$};    
  \node[bigPerm] (perm4_node) at (18,0.3*\vnya+0.7*\vnyb) {\Large $\pi_{f(w_1^{\mathrm{p}})}$};
\end{tikzpicture}