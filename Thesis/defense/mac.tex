...
\begin{itemize}
\item Identical encoder for all the users. Solely depends on the message
\item Two stages of encoding:
	\begin{itemize}
	\item encoder within a slot: Code $\mc{C}$ good over $T$-user GMAC
	...
	\end{itemize}
\item Peeling based SIC decoding across slots
\end{itemize}
}
\only<3>{
\centering
...
\begin{block}{Main result}
	\begin{itemize}
		\item Polynomial encoding and decoding complexity: $\mc{O}(KT^6+n)\approx \mc{O}\left(\max(K,n)\right)$
