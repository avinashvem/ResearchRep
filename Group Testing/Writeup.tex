...
\begin{theorem}
Let $p\in\Z$ such that $K$ and $N$ scale as $K\in o(N^{\frac{p}{p+1}})$. For $m=(2p+2)C(\epsilon)K \log_{2}\frac{N}{K}$, the regular SAFFRON framework we proposed based on left-and-right-regular sparse graphs asymptotically recovers atleast a $(1-\epsilon)$ fraction of the defective items for arbitrarily-small $\epsilon$ with high probability $1-O(\frac{K^{p+1}}{N^p})$. Note that computational complexity of the decoding scheme is $O(K\log \frac{N}{K})$. The constant $C(\epsilon)$ is given in Table. \ref{Table:constantsDE}.
\end{theorem}
\begin{proof}
