...
\begin{align*}
\mbb{P}[\mc{E}_{21}]\leq ... %6e^{-\frac{N^{\mu-\alpha}(1-6\eta)^2}{16}}
\end{align*}
\end{lemma}
...
\[
\mathbf{Z_{i,j}} = \begin{bmatrix}
\underline{w}^{j_{1}},\underline{w}^{j_2}, & \cdots   & \underline{w}^{j_p}, &\cdots \ &\underline{w}^{j_{f_i}}
\end{bmatrix} \times
    \begin{bmatrix}
n_{1} \\
\vdots \\
...
\end{bmatrix}
\]
where for convenience we use a simpler indexing notation $j_k=j+(k-1)\frac{N}{f_i}$ and $n_{l}=\sum\limits_{k=0}^{M-1}x[\theta_{\ell}+k]y[k]$ as defined in Eq. \eqref{Eqn:BinCombination}.
