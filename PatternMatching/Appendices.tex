...
\begin{lemma}
Let us consider $r[\theta_0],r[\theta_0+f_i],\ldots ,r[\theta_{0}+(n_i-1)f_i]$ where $\theta_0+kf_i \notin \{\tau_1,\ldots, \tau_L\}$ are not one of the matching positions. Then for any $\delta>0$:
\begin{align*}
\text{\textbf{Upper Tail}}: &\mbb{P}\left[ \left(\sum\limits_{i=0}^{n_i-1}\sum\limits_{k=0}^{M-1}x[\theta_i+k]y[k]\right)\geq M\delta\right]\leq \exp\left\lbrace-\frac{M\delta^2}{2n_i}\right\rbrace
\end{align*}
\end{lemma}
\begin{proof}
Then $\mbb{P}[x[\theta_i+k]y[k]=+1]$ with probability 1/2 and $\mbb{E}[x[\theta_i+k]y[k]]=0$. We will use the fact that $\alpha>\mu$ from Eqn. \eqref{Eqn:IneqMuAlpha} and thus $f_i>M$ and hence these terms are non-overlapping. Thus we can show the independence and hence we can apply the bounds from Lem. \ref{Lem:Chernoff2}.
\end{proof}
