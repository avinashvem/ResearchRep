...
\label{sec:mainresults}
	...
	\begin{itemize}
		\item a sketching function for $\yv$ that computes S samples given by
		
		\begin{equation} \label{eqn:RXY_sparse}
		S \ = \left\{
		\begin{array}{ll}
		&O\left(N^{1-\mu}\log N\right),~~  \forall \mu \leq 0.5 \\
		& O\left(N^{1/2}\log N\right),~~ \forall \mu > 0.5
		\end{array}
		\right.
		\end{equation}.
		\item a computational complexity of
			...
			C \ = \left\{
			\begin{array}{ll}
			&O\left(\max\{N^{1-\mu}\log^2 N, N^{\mu+\lambda}\log N \}\right),~~  \forall \mu \leq 0.5 \\
			& O\left(N^{1/2}\log^2 N\right),~~ \forall \mu > 0.5
			\end{array}
			\right.
			\end{equation}.
