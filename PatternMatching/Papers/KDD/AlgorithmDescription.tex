...
\paragraph*{Singleton decoding}
 {\color{blue}{(Change to the clustering scheme)}}If a bin node $(i,j)$ is classified as a singleton, we need to identify the position of the non-zero variable node connected to it. This is done by correlating the observation vector $\zv_{i,j}$ with each column of the sensing matrix  $\mb{W}_{i,j} \coleq [\wv^{j},\wv^{j+g_i},\ldots,   \wv^{j+(f_i-1)g_i}]$ and choose the index that maximizes the correlation value.
\begin{align*}
