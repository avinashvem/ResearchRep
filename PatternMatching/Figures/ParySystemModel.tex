...
\node (chan)  [right = \ext of enc,draw,rectangle,text centered,thick,minimum width=1.5in,minimum height=1.5in]    {\includegraphics[width=1.5in,angle=-90]{/Users/avinashvem/Dropbox/NASIT2016/Figures/BECchannelmodel.pdf}};
\node (dec)[rect,right=\ext of chan] {\fsizes Decoder} ;

\draw[<-,thick](enc)-- +(-2*\ext,0) node[midway,above]{\fsize $m_1,\ldots,m_l$};
\draw[->,thick](enc)--(chan)node[midway,above]{\fsize $c_1,\ldots,c_n$}node[midway,below]{\fsize $c_i\in \mathbb{F}_p$};
...
\draw[->,thick](dec)--+(2*\ext,0)node[midway,above]{\fsize  $\hat{m_1},\ldots,\hat{m_l}$};
