...
\begin{figure}
	\centering
	\scalebox{0.375}{\input{Pattern_matching_ex.tex}}
\end{figure}
\vspace{-6pt}
	\begin{block}{}
...
\begin{figure}
	\centering
	\scalebox{0.45}{\input{Pattern_matching_ex.tex}}
\end{figure}
	\vspace{-8pt}
	{\small
	\begin{table}[h!]
...
%						\end{align}
	...
	$N=6$, and the non-zero coeffs be r[0] = 5, r[1] = 3 , r[2]=1, r[3] = 4, r[4] = 7.
	     \end{block}}
     	\begin{columns}
     	\only<3->{
     		\column{0.43\textwidth}
...
\begin{frame}\frametitle{RSIDFT framework}
    ...
    \item $N = f_1 \ f_2 \ldots \ f_d$ where $f_i \approx {\color{red}{N^\alpha}}$, {\color{red}$\alpha$ depends on $M$}
...
\begin{lemma}[Mutual Incoherence Bound for sub-sampled IDFT matrix (Pawar'14)]
	...
	The mutual incoherence $\mu_{\text{max}}$ $(\mathbf{W}_{i,k})$ of the sensing matrix $\mathbf{W}_{i,k}$, with columns and rows sampled from IDFT matrix, and  $B$ shifts, is upper bounded by
...
\begin{itemize}
\item successfully uncovers all the $L$ matching positions if $L=c_2 N^{\alpha}$ and $c_2 < 1$, with probability at least $1-O(1/N^{\frac{1}{d}})$
...
\end{frame}
%----------------------------------------------------------------------------------------------------------------
\begin{frame}{Computation Tree}
\begin{figure}
  \centering
  \includegraphics[width=4.0in]{ComputationTree}
\end{figure}
\begin{itemize}
  \item $p_j$ is the prob. that a node at stage $j$ is unrecovered
  \item $p_{j-1} = f(p_j,d,L,M)$ and we want $p_0 \rightarrow 0$
  \item Analyze the average behavior over the ensemble of graphs
\end{itemize}
\end{frame}
%----------------------------------------------------------------------------------------------------------------
