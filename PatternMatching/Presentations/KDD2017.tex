...
\author{ \alert{Nagaraj T. Janakiraman}\\
Joint work with {\color{blue} A. Vem , K. R. Narayanan and J. F. Chamberland} \\
}
\titlegraphic{
...
\input{../Figures/datacomputingcomparison}
...
%\begin{itemize}
%  \item Suppose a database of genomes contains a sequence $\vec{x}$ of length $N=10^{6}-10^{12}$
...
\begin{block}{Metabolomics}
\begin{itemize}
  \item Suppose a database of genomes contains a sequence $\vec{x}$ of length $N=10^{12}$.
  \item DNA sequences of length $M=1000$ to $M = 100000$ in length (e.g., Oxford Nanopore technology can provide long reads)
  \item $\mu = 0.25$ to $\mu = 0.42$
\end{itemize}
...
\begin{frame}{Example}
	...
	\begin{figure}[t]
		\centering
		\includegraphics[width=4.5in]{Example_full_framework}
	\end{figure}
...
\begin{frame}{Properties of Fourier Transform - A Quick Review}
	\begin{block}{Shift Property}
		\begin{itemize}
			\item Let $x[n] \xrightarrow{N-DFT} X[k] $ and  $w = e^{\frac{-j2\pi}{N} }$,
		\end{itemize}
		 \begin{center}
		 	$\boxed{x[n-n_0] \xrightarrow{N-DFT} w^{k n_0}X[k]}$
		 \end{center}
	\end{block}
...
\only<2>{ \begin{block}{IDFT Computation($N=6 = 3 \times 2$)}
		\begin{figure}[t]
			\centering
			\includegraphics[width=3.1in]{X_DFT}
		\end{figure}
...
\begin{frame}\frametitle{RSIDFT Framework}
    ...
    \end{itemize}
		\begin{figure}[t!]
			\begin{center}
				\resizebox{0.75\textwidth}{!}{\input{FFAST_Robust_PM.tex}}
...
%%	}	
%	
%		\only<1>{\begin{block}{Error Probability}
%			\begin{align*}
...
\begin{frame}\frametitle{Conclusions}
	\begin{itemize} \itemsep10pt
		\item Algorithm that performs Pattern Matching in {\color{blue}sub-linear} time and space complexity
		\item {\color{blue} Signal Processing} approach to the problem
		\item Future work:
		    \begin{itemize}
		    	\item Secure Pattern Matching
		    	\item Pattern Matching on Graphs
