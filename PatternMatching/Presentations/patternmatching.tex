...
\only<2>{ \begin{block}{IDFT Computation($N=6$)}
	...
	\begin{columns}
		...
		\column{.47\textwidth}
		\begin{block}{{\small $\color{red}x_s$:\ Sub-sampled by $P_1=2$ (shifted)}}
			\begin{figure}[t]
				\centering
...
\only<2>{\vspace{-5pt} \begin{block}{Position Identification}
		\begin{itemize}
			...
			\zv_{i,k}= \begin{bmatrix}
			e^{\frac{j2\pi k s_1}{N}} & e^{\frac{j2\pi (k+f_i) s_1}{N}} & \ldots & e^{\frac{j2\pi (k+(g_i-1)f_i) s_1}{N}} \\
			e^{\frac{j2\pi k s_2}{N}} & e^{\frac{j2\pi (k+f_i) s_2}{N}} & \ldots & e^{\frac{j2\pi (k+(g_i-1)f_i) s_2}{N}} \\
			\vdots & \vdots & \ddots & \vdots\\
			e^{\frac{j2\pi k s_B}{N}} & e^{\frac{j2\pi (k+f_i) s_B}{N}} & \ldots & e^{\frac{j2\pi (k+(g_i-1)f_i) s_B}{N}} \\
			\end{bmatrix} \times
			\begin{bmatrix}
			r[k+(0)f_i] \\
			r[k+(1)f_i] \\
