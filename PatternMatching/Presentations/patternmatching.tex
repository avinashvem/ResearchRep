...
%--------------------------------------------- Main Result -------------------------------	
    ...
    \vspace{-2.5mm}
	 ...
	 \begin{block}{\alert{Note}}
	 	Particularly when $L=O(1)$ or $L<\frac{N}{M}$ (i.e. $\lambda<1-\mu$) our algorithm has a {\color{blue}sub-linear} time and space complexity.
	 \end{block}	 
\end{frame}	 
...
\begin{frame} \frametitle{Some Prior Work}
	...
	\only<2>{\begin{block}{\alert{Sparse Fourier Transform Approach}}
			\begin{itemize}
...
%		}
	...
	\only<2>{\begin{block}{$\mathcal{E}_2${-\it Pos. Identification}}
			\begin{itemize}
				\item $\underline{z}_{i,j} =  r[j_p] ~ \wv_{j_p}+ \sum_{k \neq p}n_k \wv_{j_k}$
				\item Mutual Incoherence property to bound the cross-correlation(noise) term
				  \begin{itemize}
				  	\item [-] $\log N$ measurements (shifts) suffices
				  \end{itemize}
			\end{itemize}
		\end{block}
