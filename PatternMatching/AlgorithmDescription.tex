...
\vspace{\gap}
  \State $(\hat{k},\hat{r}[\hat{k}])=${\bf Singleton-Decoder}$(i,j)$.%{$()\widehat{\msc{H}}_{i,j}=\msc{H}_s(\hat{k},\hat{r}[\hat{k}])$}   
\vspace{\gap}
...
\vspace{\gap}
      ...
      \EndFor
%  \State Remove the bin and all variables connected
%\EndIf
\EndWhile
\end{algorithmic}
\end{algorithm}
