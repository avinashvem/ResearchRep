...
\subsubsection{Singleton decoding}
 If a bin node $(i,j)$ is classified as a singleton, we need to identify the position of the significant non-zero variable node connected to it. This is done by correlating the observation vector $\rv^{b}_{i,j}$ with each column of  $\mb{W}_{i,j} \coleq [\underline{w}^{j},\underline{w}^{j+n_i},\ldots,   \underline{w}^{j+(f_i-1)n_i}]$
 %, where $\underline{g}_l =\begin{bmatrix}
	%		 e^{j \frac{2\pi l s_1}{N}}, ~ e^{j \frac{2\pi l s_2}{N}}, ~ \cdots\, ~ e^{j \frac{2\pi l s_B}{N}}
		%	 \end{bmatrix}^{T}$
and choose the index that maximizes the correlation value.
\begin{align*}
...
\end{align*}
 ...
 $$
 where $K$ is defined earlier in the problem statement. Note that though the actual value of $x[k]\in\{M-K,\ldots,M-1,M\}$ and our estimate $ \hat{r}[\hat{k}]=\frac{M-K}{2}$ is only a loose approximate, this would suffice for our purposes as we are mainly interested in recovering only the positions of matches. \\
...
\vspace{\gap}
  ...
  \frac{M-K}{2} & \text{ Approximate Matching case}
  \end{cases}
  $$
%  \vspace{\gap}
