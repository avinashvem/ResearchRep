	...
	\begin{columns}
		...
		\begin{block}{Peeling decoder}
			\begin{itemize}
				...
				%\item RS is over the complex field, so error detection is perfect
	...
	\only<1-3>{\begin{block}{Example 1}
		Let $N=6$, and the non-zero coefficients be X[0]=5, X[3]=4, X[4]=7. Is this recoverable?
	    \end{block}	}
	    \vspace{-7pt}
		\begin{columns} 
			\only<2-3>{
	...
	\only<4-6>{
		...
		Let $N=6$, and the non-zero coefficients be X[0] = 5,X[1] = 3 , X[2]=1, X[3] = 4, X[4] = 7. Is this recoverable?
	     \end{block}}
	     \begin{columns}	
